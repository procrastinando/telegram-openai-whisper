            ...
            if "message" in i:
                ...
                if len(white_list) == 0 or user_id in white_list:
                    ...
                    try: # open user settings, if it does not exist, use default settings
                        ...
                        db_settings[user_id]["first_name"] = i["message"]["chat"]["first_name"]
                        ...
                        if i["message"]["text"] in settings: # If is a command
                            ...
                            if settings["position"] == "/source_lang":
                                if i["message"]["text"] == "english":
                                    ...
                                    send_message(user_id, "Source language: english")
                                    settings["position"] = ""
                                else:
                                    settings["/source_lang"] = 'Automatic'
                                    send_message(user_id, "Source language: Automatic")
                                    settings["position"] = ""
                                ...
                                try:
                                    ...
                                    send_message(user_id, "Target language: " + i["message"]["text"])
                                    settings["position"] = ""
                                except:
                                    send_message(user_id, "Language not found!")
                                ...
                                try:
                                    ...
                                    send_message(user_id, "Voice: " + i["message"]["text"])
                                    settings["position"] = ""
                                except:
                                    send_message(user_id, "Voice not found!")
                            
                            elif settings["position"] == "/model_size":
                                dic_models = ["tiny", "base", "small", "medium", "large"]
                                try:
                                    settings["/model_size"] = i["message"]["text"]
                                    send_message(user_id, "Model: " + i["message"]["text"])
                                    settings["position"] = ""
                                except:
                                    send_message(user_id, "Model not found!")

                            elif settings["position"] == "/conversation_mode":
                                if i["message"]["text"] in settings["list"]:
                                    pass
                                elif i["message"]["text"] == "Deactivate":
                                    pass
                                else:
...
def set_menu(user):
    base_url_download = base_url + '/setChatMenuButton'
    headers = {"Content-Type": "application/json"}
    data = {
        "chat_id": user,
...
def comdandos():
    base_url_download = base_url + '/getMyCommands'
    headers = {"Content-Type": "application/json"}
    data = {
        "scope": "default",
        }
...
def update_commands(user, db_settings):
    commands_array = [{"command": "/source_lang", "description": "Automatic"}, {"command": "/target_lang", "description": ""}, {"command": "/auto_trans", "description": "Deactivated"}, {"command": "/auto_speech", "description": "Deactivated"}, {"command": "/conversation_mode", "description": "Deactivate"}, {"command": "/voice", "description": ""}, {"command": "/model_size", "description": ""}]
    
    commands_array[0]["description"] = inverted_dic_abb[db_settings[user]["/source_lang"]]
    commands_array[1]["description"] = inverted_dic_abb[db_settings[user]["/target_lang"]]
    ...
    commands_array[4]["description"] = db_settings[user]["/conversation_mode"]
    
    dic_voices = []
    for j in engine.getProperty('voices'):
        dic_voices.append(j.name)
    commands_array[5]["description"] = dic_voices[db_settings[user]["/voice"]]

    commands_array[6]["description"] = db_settings[user]["/model_size"]
